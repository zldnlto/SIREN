...
def test_sampling_manifest_excludes_taxonomy_review_images_from_train_by_default() -> None:
    sampling_config = SamplingConfig(
        regular_min_unique_images=1,
        tail_min_unique_images=1,
        regular_train_cap=10,
        tail_train_cap=10,
        review_train_cap=0,
        validation_policy="natural",
    )
    records = [
        ...
        ImageSplitRecord(
            ...
            ontology_id="v1.표면처리.균열.도장.defect",
            quality_state="defect",
            taxonomy_statuses=("taxonomy_review_required",),
            label_types=("bbox",),
            task_types=("detect",),
            original_category_ids=(25,),
            zip_sources=("TL_표면처리_균열_도장.zip",),
    ...
    assert report.taxonomy_review_image_count == 1
    assert any(item.file_name == "review.jpg" and item.selection_reason == "taxonomy_review_excluded" for item in selections)
    assert all(not item.selected_for_train for item in selections if item.file_name == "review.jpg")
