...
```python
missing_df = df[df["file_name"].isin(set(missing))]
print(missing_df["domain"].value_counts())
print(missing_df["zip_source"].value_counts().head(10))
```

### 조치 방향

- **단기**: 표면처리 도메인만으로 베이스라인 학습 진행 (누락 0)
- **중기**: 나머지 도메인 이미지의 실제 Drive 경로 확인 후 `SOURCE_DIR` 목록 확장
